...
2. **`src/send.mjs`** (daily, GitHub Actions) asks the API for the next NEW batch
   ...
   from **`getpeekscout.com`** (never the primary `peekscout.com` domain), and
...
| `RESEND_API_KEY` | Resend key for the account where **getpeekscout.com is a verified domain** |
...
| `OUTREACH_FROM_EMAIL` | `Peek Scout <hello@getpeekscout.com>` |
...
- Sends **only** from `getpeekscout.com` — refuses any other From domain.
